...
def add_rolling_mean(df: pd.DataFrame, columns: list, window=3) -> pd.DataFrame:
    ...
    for i in range(dist_from_center):
        df.loc[df.index[0] - df.index[i + 1] + df.index[0], :] = df.iloc[
            max_index - i, :
        ]
...
class OverTime(metaclass=ABCMeta):
    ...
    def plot(
        ...
        ----------
        nticks : int, optional
            Ticks on the y-axis, by default 6
        kind : Literal["delay", "cancellations", "on_time_percentage"], optional
            What to plot, by default 'delay'
        save_as : Optional[str], optional
            Whether and where to save plot to disk, by default None
        ...
        if kind not in ['delay', 'cancellations', 'on_time_percentage']:
            raise ValueError(
                f'kind must be "delay", "cancellations" or "on_time_percentage" not {kind}'
            )
        ...
        if kind == 'delay':
            ax1.plot(
                self.data['ar_happened_sum'] + self.data['dp_happened_sum'],
                color='blue',
                linewidth=3,
                label='Stops',
            )
            ...
            ax2.plot(
                self.data['ar_delay_mean'],
                color='red',
                linewidth=3,
                label='Arrival delay',
            )
            ax2.plot(
                self.data['dp_delay_mean'],
                color='orange',
                linewidth=3,
            ...
            ax2.plot(
                (self.data['ar_on_time_mean']).round(3) * 100,
                color='red',
                linewidth=3,
                label='Arrival',
            )
            ax2.plot(
                (self.data['dp_on_time_mean']).round(3) * 100,
                color='orange',
                linewidth=3,
                label='Departure',
            )
            ...
            ax2.plot(
                self.data['ar_happened_mean'],
                color='red',
                linewidth=3,
                label='Arrival non-cancellations',
            )
            ax2.plot(
                self.data['dp_happened_mean'],
                color='orange',
                linewidth=3,
...
class OverHour(OverTime):
    ...
    def periodic_aggregator(self, rtd: dd.Series) -> dd.Series:
        return datetime.datetime(year=2000, month=1, day=3) + dd.to_timedelta(
            rtd.dt.minute, unit='m'
        )
...
if __name__ == '__main__':
    ...
    time.plot(save_as='over_hour_cancellations.png', kind='cancellations')

    print('grouping over day')
    time = OverDay(generate=False)
    time.plot(save_as='over_day_cancellations.png', kind='cancellations')

    print('grouping over week')
    time = OverWeek(generate=False)
    time.plot(save_as='over_week_cancellations.png', kind='cancellations')

    print('grouping over year')
    time = OverYear(generate=False)
    time.plot(save_as='over_year_cancellations.png', kind='cancellations')

    print('grouping over year')
    time = OverYearWeekly(generate=False)
    time.plot(save_as='over_year_weekly_cancellations.png', kind='cancellations')
