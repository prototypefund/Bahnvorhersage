...
| May 2022                    | Beginning on the fist of june, Germany introduced the 9€-Ticket for local trains. So we needed a way to route for local trains only. That was easy, as Marudor's API has support for that. However, we found that if we added more options to our search, we should also add an option to search for routes with bikes. And that is not supported by Marudor. So we switched our routing provider once again an moved to a self hosted instance of https://v5.db.transport.rest/. For that we had to rewrite all of our server parsing and rendering code. At least now we use the Friendly-Public-Transit-Format |
