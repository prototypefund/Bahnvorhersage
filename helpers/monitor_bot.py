...
def monitor_plan() -> str | None:
    ...
    try:
        ...
        if plan_count_delta < 500:
            message = f'@everyone The plan gatherer is not working, as {str(plan_count_delta)} new entries where added to database at {str(date_to_check)}'
        print(
            'checked plan '
            + str(date_to_check)
            + ': '
            + str(plan_count_delta)
            + ' rows were added'
        )
...
def monitor_change() -> str | None:
    ...
    try:
        ...
        if count_delta < 1000:
            message = f"""@everyone The recent change gatherer is not working, as {str(count_delta)} 
                    new entries where added to database at {str(date_to_check)}"""
        old_change_count = new_change_count
        print(
            'checked changes '
            + str(date_to_check)
...
async def monitor_website():
    ...
    try:
        ...
        if page.ok:
            ...
            message = (
                ...
                + f': @everyone Something not working on the website:\n{str(page)}'
            )
            print(message)
        ...
        message = (
            str(datetime.datetime.now())
            + f': @everyone Error on the website:\n{str(ex)}'
        )
        print(message)
        await channel.send(message)
    ...
    try:
        print(
            'testing https://bahnvorhersage.de/api/trip from Tübingen Hbf to Köln Hbf...'
        )
        search = {
            'start': 'Tübingen Hbf',
            'destination': 'Köln Hbf',
            'date': (datetime.datetime.now() + datetime.timedelta(hours=1)).strftime(
                '%d.%m.%Y %H:%M'
            ),
